...
export function ScheduledMessagesTable({
  ...
  const { messages, pagination } = useMemo(() => {
    ...
    const data =
      ...
      [];
    const paginationData: PaginationData = (tableDataAny.pagination as PaginationData) ||
      ((tableDataAny.message as Record<string, unknown>)?.pagination as PaginationData) || {
        current_page: (tableDataAny.current_page as number) || 1,
        last_page: (tableDataAny.last_page as number) || 1,
        per_page: (tableDataAny.per_page as number) || perPage,
        total: (tableDataAny.total as number) || 0,
        from: (tableDataAny.from as number) || 0,
        to: (tableDataAny.to as number) || 0,
      };
    return {
                  ...
                  <TableHeader>
                    <TableRow className="border-b-2 border-gray-200 bg-gradient-to-r from-gray-50 via-gray-50/50 to-gray-50 dark:border-gray-700 dark:from-gray-800 dark:via-gray-800/50 dark:to-gray-800">
                      ...
                      <TableHead className="w-[140px] px-2 py-3 text-xs font-bold uppercase tracking-wider text-gray-700 dark:text-gray-200 md:py-4 md:text-sm">
                        <button
                          onClick={() => handleSort("bulk_sms.msisdn")}
                          className="flex items-center gap-1 transition-colors hover:text-brand-500"
                        >
                          Phone
                          {(() => {
                            const direction = getSortDirection("bulk_sms.msisdn");
                            if (direction === "asc") return <ArrowUp className="h-3 w-3" />;
                            if (direction === "desc") return <ArrowDown className="h-3 w-3" />;
                            return <ArrowUpDown className="h-3 w-3 opacity-50" />;
                          })()}
                      ...
                      <TableHead className="hidden w-[120px] px-2 py-3 text-xs font-bold uppercase tracking-wider text-gray-700 dark:text-gray-200 sm:table-cell md:py-4 md:text-sm">
                        <button
                          onClick={() => handleSort("bulk_sms.sender_id")}
                          className="flex items-center gap-1 transition-colors hover:text-brand-500"
                        >
                          Sender ID
                          {(() => {
                            const direction = getSortDirection("bulk_sms.sender_id");
                            if (direction === "asc") return <ArrowUp className="h-3 w-3" />;
                            if (direction === "desc") return <ArrowDown className="h-3 w-3" />;
                            return <ArrowUpDown className="h-3 w-3 opacity-50" />;
                          })()}
            ...
            {pagination && typeof pagination.total === "number" && pagination.total > 0 && (
              <div className="border-border/50 mt-4 flex flex-col items-center justify-between gap-4 border-t pt-4 dark:border-gray-700 sm:flex-row md:mt-6">
                <div className="text-xs text-gray-600 dark:text-gray-400 md:text-sm">
                  Displaying {typeof pagination.from === "number" ? pagination.from : 0} to{" "}
                  {typeof pagination.to === "number" ? pagination.to : 0} of {pagination.total}{" "}
                  messages
                </div>
                <div className="flex items-center gap-2">
                  <Button
                  ...
                  <div className="px-3 text-xs font-medium text-gray-700 dark:text-gray-300 md:px-4 md:text-sm">
                    Page{" "}
                    {typeof pagination.current_page === "number" ? pagination.current_page : page}{" "}
                    of {typeof pagination.last_page === "number" ? pagination.last_page : 1}
                  </div>
                  <Button
                    variant="outline"
                    size="icon"
                  ...
                  <Button
                    ...
                    size="icon"
                    onClick={() =>
                      setPage(typeof pagination.last_page === "number" ? pagination.last_page : 1)
                    }
                    disabled={
                      isLoading ||
                      (typeof pagination.last_page === "number"
